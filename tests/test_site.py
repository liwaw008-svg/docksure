...
def test_real_complete_frontend():
 ...
 assert all(x in j for x in ('open_shipment','accept_shipment','submit_delivery','recover_unsettled','waitForTransactionReceipt','policy()'))
 assert 'allow1' in h and 'allow2' in h and 'CUSTOMER-BOUND EVIDENCE POLICY' in h
