...
try:
    kc.simulate_write_contract(address=address,function_name='submit_delivery',args=[shipment,[sources[0],sources[0]+'/duplicate']])
    raise RuntimeError('same-slot evidence simulation unexpectedly passed')
except Exception as e:print('negative','same-slot evidence rejected',flush=True)
